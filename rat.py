...
def block_usb():
    """Блокирует все USB-накопители (флешки)"""
    global usb_blocked
    try:
        # Через реестр
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 
            r"SYSTEM\CurrentControlSet\Services\USBSTOR", 0, winreg.KEY_WRITE)
        winreg.SetValueEx(key, "Start", 0, winreg.REG_DWORD, 4)  # 4 = отключено
        winreg.CloseKey(key)
        usb_blocked = True
...
def unblock_usb():
    """Разблокирует USB-накопители (флешки)"""
    global usb_blocked
    try:
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, 
            r"SYSTEM\CurrentControlSet\Services\USBSTOR", 0, winreg.KEY_WRITE)
        winreg.SetValueEx(key, "Start", 0, winreg.REG_DWORD, 3)  # 3 = включено
        winreg.CloseKey(key)
        usb_blocked = False
...
def usb_status():
    """Показывает статус USB-блокировки"""
    global usb_blocked
