...
def callback(update, context):
    ...
    if data.startswith("select_pc|"):
        ...
        if pc_id in known_pcs:
            selected_pc = pc_id
            bot.send_message(chat_id, f"✅ Выбран ПК: `{pc_id}`", parse_mode="Markdown")
        else:
            bot.send_message(chat_id, f"❌ ПК `{pc_id}` не найден", parse_mode="Markdown")
        return
    ...
    if data == "screenshot":
        ...
        bot.send_message(chat_id, enable_dark_mode())
    elif data == "light_mode":
        bot.send_message(chat_id, disable_dark_mode())
