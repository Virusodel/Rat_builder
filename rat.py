...
# ============ ИСПРАВЛЕННЫЕ ВИЗУАЛЬНЫЕ ЭФФЕКТЫ (GDI) ============
...
def gdi_effect_zoom():
    ...
    try:
        ...
        def update():
            try:
                img = ImageGrab.grab()
                # Плавное изменение размера для эффекта зума
...
def pixellate_screen():
    """Исправленная пикселизация экрана (без белого экрана)"""
    try:
        import tkinter as tk
        root = tk.Tk()
        root.attributes('-fullscreen', True, '-topmost', True)
        # Важно: устанавливаем черный фон
        root.configure(bg='black')
        ...
        def update():
            try:
                ...
                # Увеличиваем обратно с NEAREST для пиксельного эффекта
                big = small.resize(img.size, Image.Resampling.NEAREST)
                tk_img = ImageTk.PhotoImage(big)
                label.config(image=tk_img)
                label.image = tk_img
